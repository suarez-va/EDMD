...
xmat = dvr_xn(1, a, b, N, "(a,b)")
Tmat = dvr_T(m, a, b, N, "(a,b)")
Wmat = dvr_W(a, b, N, acap=-7.0, bcap=7.0, eta=1.0e-6,n=2, bounds="(a,b)")
print(np.diag(Wmat))
...
pot=-D*np.exp(-A*x**2)
#pot=V0*(np.heaviside(x+0.5*L,0.5)-np.heaviside(x-0.5*L,0.5))
...
plt.plot(x,np.abs(C[:,3])**2,color='k')
#plt.plot(x,np.abs(C[:,4])**2,color='violet')
#plt.plot(x,np.abs(C[:,7])**2)
plt.xlim([a,b])
plt.savefig("wfn.png")
