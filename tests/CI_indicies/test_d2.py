...
from time_independent.fcidvr import FCIDVR, solve_d2En
...
R_sub = 3.000384615384615
#nbo_sub = 225
nbo_sub = 225

fcidvr.kernel(R = R_sub, nele = 1, spin = 'doublet', nbo = nbo_sub)
fcidvr.kernel(R = R_sub, nele = 2, spin = 'singlet', nbo = nbo_sub)

#dat1 = np.load("fcidvr_1ele_doublet.npz")
dat2 = np.load("fcidvr_2ele_singlet.npz")
#En = dat1['En']; Cn = dat1['Cn']; d1En = dat1['d1En']; d1Cn = dat1['d1Cn']; nac1 = dat1['nac1']; d2En = dat1['d2En']; d2Cn = dat1['d2Cn']; nac2 = dat1['nac2']
En = dat2['En']; Cn = dat2['Cn']; d1En = dat2['d1En']; d1Cn = dat2['d1Cn']; nac1 = dat2['nac1']; d2En = dat2['d2En']; d2Cn = dat2['d2Cn']; nac2 = dat2['nac2']

print(np.max(np.absolute(np.diag(Cn.conj().T @ d1Cn))))
print(np.max(np.absolute(np.diag(Cn.conj().T @ d2Cn))))
print(np.max(np.absolute(np.diag(d1Cn.conj().T @ d2Cn))))

nac1_test = Cn.conj().T @ d1Cn
print(nac1_test[10,12])
print(nac1[10,12])

nac2_test = Cn.conj().T @ d2Cn
print(nac2_test[10,12])
print(nac2[10,12])

print(np.absolute(nac2_test[10,12]))
print(np.absolute(nac2[10,12]))


exit()


d1H = fcidvr.CIoperator(nele = 1, spin = 'doublet', hij = fcidvr.d1hij(R = R_sub))
d2H = fcidvr.CIoperator(nele = 1, spin = 'doublet', hij = fcidvr.d2hij(R = R_sub))
dat = np.load("fcidvr_1ele_doublet.npz")
En = dat['En']; Cn = dat['Cn']
d1Hnm = Cn.conj().T @ d1H.matmat(Cn) + fcidvr.model.d1VR(R = R_sub) * np.eye(nbo_sub)
d2Hnm = Cn.conj().T @ d2H.matmat(Cn) + fcidvr.model.d2VR(R = R_sub) * np.eye(nbo_sub)
d1En = np.diag(d1Hnm).real
d2En = solve_d2En(En, d1Hnm, d2Hnm)

hij = fcidvr.hij(R = R_sub)
d1hij = fcidvr.d1hij(R = R_sub)
d2hij = fcidvr.d2hij(R = R_sub)
ep, cip = eigh(hij, subset_by_index=[0,nbo_sub-1])
d1hpq = cip.conj().T @ d1hij @ cip + fcidvr.model.d1VR(R = R_sub) * np.eye(nbo_sub)
d2hpq = cip.conj().T @ d2hij @ cip + fcidvr.model.d2VR(R = R_sub) * np.eye(nbo_sub)
d1ep = np.diag(d1hpq).real
d2ep = solve_d2En(ep, d1hpq, d2hpq)

n = 2
print("d1E:")
print(f"from kernel: {d1En[n]}")
print(f"from eigh: {d1ep[n]}")

print("d2E:")
print(f"from kernel: {d2En[n]}")
print(f"from eigh: {d2ep[n]}")

#print(f"from kernel: {d2Hnm}")
#print(f"from eigh: {d2hpq}")
