...
#model = TEGD(a=-100, b=100, N=500, bounds="(-inf,inf)", spin="singlet", model_params=params)
model = TEGD(a=-200, b=200, N=1000, bounds="(-inf,inf)", spin="singlet", model_params=params)
ep, cip = model.solve_mos(R = 8.0)
ci1 = cip[:,1]
ci2 = cip[:,2]
S12 = 1.0 / np.sqrt(2) * (ci1[:,None] * ci2[None,:] + ci2[:,None] * ci1[None,:])

St0 = np.exp(-1j*np.pi/2.1) * model.wfn_to_vec(S12)
Hop = model.H(R = 8.0)
