...
Hele = generate_Hele(6.60000000000, params)
E, C = np.linalg.eigh(Hele)

dHele = np.linalg.multi_dot([np.conj(C.T), generate_dHele(6.60000000000, params), C])
nac1 = calculate_nac(E, dHele)
nac2 = np.einsum('ij,jk->ik', nac1, nac1)
dipole = np.linalg.multi_dot([np.conj(C.T), generate_dipole(6.60000000000, params), C])
cap = np.linalg.multi_dot([np.conj(C.T), generate_cap(6.60000000000, params), C])
