...
Hele = generate_Hele(14.10000000000, params)
E, C = np.linalg.eigh(Hele)

dHele = np.linalg.multi_dot([np.conj(C.T), generate_dHele(14.10000000000, params), C])
nac1 = calculate_nac(E, dHele)
nac2 = np.einsum('ij,jk->ik', nac1, nac1)
dipole = np.linalg.multi_dot([np.conj(C.T), generate_dipole(14.10000000000, params), C])
cap = np.linalg.multi_dot([np.conj(C.T), generate_cap(14.10000000000, params), C])
