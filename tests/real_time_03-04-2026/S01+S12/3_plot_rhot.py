...
PE2t = timedata['PE2t']
rhot = Px1t + Px2t
qLt = 1.0 - np.sum(rhot[:378,:], axis=0)
qRt = 1.0 - np.sum(rhot[378:,:], axis=0)
...
ax2.set_xlim(-0.95, 0.07)
ax2.set_ylim(-0.05, 1.05)
ax2.set_yticks([0, 1])
ax2.set_xlabel('E$_\\mathrm{i}$ (a.u.)', labelpad=-1)
ax2.set_ylabel('P(E$_\\mathrm{i}$)', labelpad=12)
...
ax3.plot(t, qt, color='k', label='total')
ax3.plot(t, qLt, color='b', label='left')
ax3.plot(t, qRt, color='r', label='right')
plot6 = ax3.axvline(x=0, color='k', linestyle='--')
plot7, = ax3.plot([],[], marker='o', color='k', markersize=7)
ax3.legend(loc='upper right')

fig.subplots_adjust(hspace=0.25, left=0.11, right=0.95, top=0.95, bottom=0.06)
